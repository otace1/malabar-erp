<?php
	$requeteVerifier = $connexion-> prepare("SELECT COUNT(d.id_dos) AS nbre
										...
										WHERE d.id_cli =  cl.id_cli
											...
											$sql1
										$sqlOrder");
	$requeteVerifier-> execute(array($entree['id_cli'], $entree['id_mod_trans'], $entree['id_mod_lic'], $entree['annee']));
	$reponseVerifier = $requeteVerifier-> fetch();

	if ($reponseVerifier['nbre']>0) {
		...
		$requeteColonne = $connexion-> prepare("SELECT c.titre_col AS titre_col, c.id_col AS id_col
		...
		$requeteColonne-> execute(array($entree['id_cli'], $entree['id_mod_lic'], $entree['id_mod_trans']));
		while ($reponseColonne = $requeteColonne-> fetch()) {
			$excel-> getActiveSheet()
				-> setCellValue($col.$row, $reponseColonne['titre_col']);
		...
		$row = 2;
		$col = 'C';
		$requeteDossierRegSusp = $connexion-> prepare("SELECT d.ref_dos AS ref_dos,
											...
											WHERE d.id_cli =  cl.id_cli
												...
												$sql1
											$sqlOrder");
		$requeteDossierRegSusp-> execute(array($entree['id_cli'], $entree['id_mod_trans'], $entree['id_mod_lic'], $entree['annee']));
		while ($reponseRegSusp = $requeteDossierRegSusp-> fetch()) {
			$compteur++;
			$bg = "";

			if ($reponseRegSusp['cleared'] == '1') {
			...
			}else if ($reponseRegSusp['cleared'] == '2') {
			...
			$date_exp = $maClasse-> getLastEpirationLicence($reponseRegSusp['num_lic']);

			$excel-> getActiveSheet()
				-> setCellValue('A'.$row, $compteur)
				-> setCellValue('B'.$row, $reponseRegSusp['ref_dos']);
			...
			afficherRowTableauExcel($id_mod_lic, $id_cli, $id_mod_trans, $reponseRegSusp['id_dos'], $compteur, $col, $excel, $row, $styleHeader);

			$row++;

		}$requeteDossierRegSusp-> closeCursor();
		...
		$requeteColonne = $connexion-> prepare("SELECT c.titre_col AS titre_col, c.id_col AS id_col
										...
										ORDER BY af.rang ASC");
		$requeteColonne-> execute(array($entree['id_cli'], $entree['id_mod_lic'], $entree['id_mod_trans']));
		while ($reponse = $requeteColonne-> fetch()) {
		...
		$excel->getActiveSheet()->setTitle('REGIME SUSPENSION '.$reponseModeTransport['nom_mod_trans'].' '.$annee);
