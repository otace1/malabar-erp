...
$indiceSheet = 0;
	...
	//--- Recuperation des Tranmission d'apurement -------
	$requeteTransmission = $connexion-> prepare("SELECT * 
													FROM transmission_apurement 
													WHERE id_trans_ap IN(
														SELECT da.id_trans_ap AS id_trans_ap
															FROM detail_apurement da, dossier d
															WHERE da.id_dos = d.id_dos
																AND d.id_cli = ?
													)");
	$requeteTransmission-> execute(array($_GET['id_cli']));
	while($reponseTransmission = $requeteTransmission-> fetch()){
		...
		$excel-> getActiveSheet()
			-> setCellValue('D4', 'TRANSMIS POUR APUREMENT '.$maClasse-> getNomClient($_GET['id_cli']).' N. '.$maClasse-> getTransmissionApurement($reponseTransmission['id_trans_ap'])['ref_trans_ap'].' DU '.$maClasse-> getTransmissionApurement($reponseTransmission['id_trans_ap'])['date_trans_ap']);
		$excel->getActiveSheet()
			->getStyle('D4')->applyFromArray($styleHeader2);

		$excel-> getActiveSheet()
			-> setCellValue('H1', 'Lubumbashi, le '.$maClasse-> getTransmissionApurement($reponseTransmission['id_trans_ap'])['date_trans_ap']);
		...
		$requete-> execute(array($reponseTransmission['id_trans_ap']));
		while ($reponse = $requete-> fetch()) {
			...
			afficherDossierLicenceTransmisionApurementExcel($reponse['num_lic'], $reponseTransmission['id_trans_ap'], $row, $excel, $compteur, $couleur);
			...
			alignement('A'.$row);*/
             ...
             if ($maClasse-> getNombreDossierApureLicence($reponse['num_lic'], $reponseTransmission['id_trans_ap']) > 1) {

				//Fusionner les cellules
				$excel-> getActiveSheet()
					-> mergeCells('C'.$row.':C'.($row+$maClasse-> getNombreDossierApureLicence($reponse['num_lic'], $reponseTransmission['id_trans_ap'])-1));
				$excel-> getActiveSheet()
					-> mergeCells('D'.$row.':D'.($row+$maClasse-> getNombreDossierApureLicence($reponse['num_lic'], $reponseTransmission['id_trans_ap'])-1));
				$excel-> getActiveSheet()
					-> mergeCells('E'.$row.':E'.($row+$maClasse-> getNombreDossierApureLicence($reponse['num_lic'], $reponseTransmission['id_trans_ap'])-1));
				$excel-> getActiveSheet()
					-> mergeCells('F'.$row.':F'.($row+$maClasse-> getNombreDossierApureLicence($reponse['num_lic'], $reponseTransmission['id_trans_ap'])-1));
				$excel-> getActiveSheet()
					-> mergeCells('G'.$row.':G'.($row+$maClasse-> getNombreDossierApureLicence($reponse['num_lic'], $reponseTransmission['id_trans_ap'])-1));

				$row += $maClasse-> getNombreDossierApureLicence($reponse['num_lic'], $reponseTransmission['id_trans_ap']);
				$compteur += $maClasse-> getNombreDossierApureLicence($reponse['num_lic'], $reponseTransmission['id_trans_ap']);
			}else{
				$row++;
				$compteur++;
			}
		...
		// Rename worksheet
		$ref_trans_ap = $maClasse-> getTransmissionApurement($reponseTransmission['id_trans_ap'])['ref_trans_ap'];
		$date_trans_ap = $maClasse-> getTransmissionApurement($reponseTransmission['id_trans_ap'])['date_trans_ap2'];
		$banque = $maClasse-> getTransmissionApurement($reponseTransmission['id_trans_ap'])['banque'];
	...
	}$requeteTransmission-> closeCursor();
	//--- FIN Recuperation des Tranmission d'apurement -------
		

$titre = $maClasse-> getClient($_GET['id_cli'])['nom_cli'].'_TRANSMIS_APUREMENT_IB_'.date('d_m_Y_h_i_s');
