<?php
  ...
  if(isset($_POST['update'])){
    ...
    for ($i=1; $i <= $_POST['nbre'] ; $i++) { 
      ...
      if (isset($_POST['date_ext_temporelle_'.$i]) && ($_POST['date_ext_temporelle_'.$i] != '')) {
        $maClasse-> MAJ_date_ext_temporelle($_POST['id_dos_'.$i], $_POST['date_ext_temporelle_'.$i]);
        // echo '<br> date_ext_temporelle = '.$_POST['date_ext_temporelle_'.$i];
        ...
        <div class="row">
          <div class="col-12">
              ...
              <div class="card card-<?php echo $couleur;?>">
                <div class="card-header">
                  <h3 class="card-title">
                    <i class="fa fa-folder-open nav-icon"></i> DOSSIERS REGIME SUSPENSION DATE ECHEANCE EXPIREE <?php echo '<span class="bg bg-dark" style="padding-left: 5px; padding-right: 5px;">'.$client['nom_cli'].'</span>'.$marchandise.$mode_transport.$commodity.$licence.$statut.$cleared;?>
                            ...
                            <table class="table table-dark  table-head-fixed table-bordered table-hover text-nowrap table-sm">
                              <thead>
                                <tr>
                                  ...
                                  <th style="border: 1px solid white; text-align: center;">Licence Num.</th>
                                  <th style="border: 1px solid white; text-align: center;">Klesa arrival date</th>
                                  <th style="border: 1px solid white; text-align: center;">Date Extrem.</th>
                                  <th style="border: 1px solid white; text-align: center;">Ref. Declaration</th>
                                  <th style="border: 1px solid white; text-align: center;">Date Declaration</th>
                              </thead>
                              <tbody>
                                <?php
                                  $maClasse-> afficherRegimeSuspensionDateExtremeExpire($_GET['id_cli'], $_GET['id_mod_trans'], $_GET['id_mod_trac']);
                                ?>
                              </tbody>
                            </table>
                          </div>
