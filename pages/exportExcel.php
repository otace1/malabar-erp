...
while ($reponseModeTransport = $requeteModeTransport-> fetch()) {
	...
	if ($_GET['id_mod_trac'] == '1') {
		...
		$requeteLicence-> execute(array($_GET['id_cli'], $reponseModeTransport['id_mod_trans'], $_GET['id_mod_trac']));
		while ($reponseLicence = $requeteLicence-> fetch()) {
			...
			while ($reponse = $requete-> fetch()) {
				...
				if ($reponse['id_col'] == '11' || $reponse['id_col'] == '13' || $reponse['id_col'] == '17') {
					...
					$excel-> getActiveSheet()-> getColumnDimension($col)-> setWidth(45);
					$col++;
				}else if ($reponse['id_col'] == '42') {
					$excel-> getActiveSheet()-> getColumnDimension($col)-> setWidth(35);
					$col++;
				}else if ($reponse['id_col'] == '43') {
					$excel-> getActiveSheet()-> getColumnDimension($col)-> setWidth(60);
					$col++;
				}else{
...
if (isset($_GET['id_march']) && ($_GET['id_march'] != '')) {
	$marchandise = '_'.$maClasse-> getMarchandise($_GET['id_march']).'_';
}else{
	$marchandise = '';
}
