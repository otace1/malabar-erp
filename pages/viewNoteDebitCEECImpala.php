...
$detail_note_debit = $maClasse-> getDetailNoteDebitCEECImpala($_GET['ref_note']);
$information = $maClasse-> getNote($_GET['ref_note'])['libelle'];
...
$dossiers3 = $maClasse-> getDossierNoteDebitCEECImpala($_GET['ref_note']);
...
$tbl = <<<EOD
	...
	<table>
		...
		<tr>
			...
			<span style="font-size: 6px;"><br>$adr_cli
			<br>No.RCCM: $rccm_cli
			<br>No.NIF.: $nif_cli
			<br>No.IDN.: $id_nat
	...
	<table  cellpadding="3">
		...
		<tr>
			<td width="100%" style="font-weight: bold; border: 1px solid black;font-size: 7px;">&nbsp;$information</td>
		</tr>
		<tr>
			...
			<td width="10%" style="font-weight: bold; border: 1px solid black; background-color: rgb(220,220,220); font-size: 7px;text-align: center;">Qty</td>
...
// add a page
$pdf->AddPage('L', 'A4');
if ( ($maClasse-> getNote($_GET['ref_note'])['validation']) == '0' ) {
	$pdf->Image('../images/no_valid.jpg', 150, 2, 30, '', '', '', '', false, 300);
}else{
	$pdf->Image('../images/signature_facture/'.$maClasse-> getDataUtilisateur($maClasse-> getNote($_GET['ref_note'])['id_util_validation'])['signature_facture'], 200, 152, $maClasse-> getDataUtilisateur($maClasse-> getNote($_GET['ref_note'])['id_util_validation'])['size_signature_facture_2'], '', '', '', '', false, 300);
...
$tbl = <<<EOD
	...
	<table>
		...
		<tr>
			<td width="5%" style=""></td>
			<td width="83%" style="">DETAILS $ref_note</td>
		</tr>
		<tr>
