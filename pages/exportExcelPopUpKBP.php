	...
	if ($_GET['id_mod_trac'] == '1') {
		...
		if (isset($_GET['id_cli']) && ($_GET['id_cli']!='')) {
			...
			$excel-> getActiveSheet()
				-> setCellValue('A1', 'KBP REPORT PREALERT DATE(S) BETWEEN '.$_GET['debut'].' AND '.$_GET['fin']);
			$excel->getActiveSheet()
			...
			$requete-> execute(array($entree['debut'], $entree['fin'], $entree['id_mod_trans'], $entree['id_mod_lic']));
			while ($reponse = $requete-> fetch()) {
				$compteur++;
				$bg = "";
...
$titre = 'KBP_PREALERT_DATES_BETWEEN_'.$_GET['debut'].'_'.$_GET['fin'].'_'.date('d_m_Y_h_i_s');
