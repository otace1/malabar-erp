...
<div class="modal fade" id="modal_client_note_debit">
  <div class="modal-dialog modal-md">
    <div class="modal-content">
      <div class="modal-header btn-dark">
        <h4 class="modal-title"><i class="fa fa-list"></i> Client </h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <div class="row">
          <input type="text" id="mot_cle" class="form-control form-control-sm" placeholder="Ex.: Mining DRC LTD" onkeyup="client_note_debit(this.value);">
          <hr>
          <div class="col-md-12 table-responsive p-0 " style="height: 500px;">
            <table class="table table-bordered table-striped text-nowrap table-hover table-sm small text-nowrap table-head-fixed ">
              <thead>
              ...
              <tbody id="table_client_note_debit">
              </tbody>
            </table>
          </div>
        </div>
...
<script type="text/javascript">
    ...
    function client_note_debit(mot_cle=null){

      $('#spinner-div').show();
      $.ajax({
        type: "POST",
        url: "ajax.php",
        data: { mot_cle: mot_cle, operation: "table_client_note_debit"},
        dataType:"json",
        success:function(data){
          if (data.logout) {
            ...
            $('#table_client_note_debit').html(data.table_client_note_debit);
            $('#modal_client_note_debit').modal('show');
          }
        },
        complete: function () {
